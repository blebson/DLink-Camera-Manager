 ...
 */
metadata {
	definition (name: "DCS-931L", author: "Ben Lebson", namespace: "blebson") {
    ...
    tiles (scale: 2) {
     multiAttributeTile(name: "videoPlayer", type: "videoPlayer", width: 6, height: 4) {
       ...
       controlTile("levelSliderControl", "device.level", "slider", height: 1, width: 5, inactiveLabel: false, range:"(0..100)") {
        ...
        main "motion"
        details(["videoPlayer", "take", "cameraDetails", "refresh", "blank", "levelSliderControl", "Sensitivity", "motion"])
    }
}
...
def start() {
	log.trace "start()"
	def dataLiveVideo = [
		OutHomeURL  : "http://${state.videoIP}:${state.videoPort}/mjpeg.cgi?user=${state.cameraUser}&password=${state.cameraPassword}&channel=1.mjpeg",
		InHomeURL   : "http://${state.videoIP}:${state.videoPort}/mjpeg.cgi?user=${state.cameraUser}&password=${state.cameraPassword}&channel=1.mjpeg",
